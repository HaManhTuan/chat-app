    ...
    <div class="wscn-http404-container">
      <div class="wscn-http404">
        ...
        <div class="bullshit">
          ...
          <a href="/" class="bullshit__return-home">トップ画面へ</a>
        </div>
      </div>
    </div>
  </div>
...
<script>
import asyncRounter from '../../router/modules/asyncRouter';
...
export default {
  ...
  created() {
    ...
    const redirect = this.$route.redirectedFrom.split('?')[0];
    const allRouters = [];
    // _.each(asyncRounter, (item) => {
    //   allRouters.push(item.path);
    // });
    // if (_.includes(allRouters, redirect)) {
    //   this.$router.push({ name: 'err_403' });
    // }
    _.each(asyncRounter, (item) => {
      if (typeof item.children !== 'undefined' && item.children instanceof Array) {
        _.each(item.children, sub => {
          if (typeof sub.accessUrl !== 'undefined'){
            allRouters.push(sub.accessUrl);
          } else {
            allRouters.push(item.path);
          }
        });
