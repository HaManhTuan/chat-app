<template>
  <div>
    <el-dialog
      ...
      :visible.sync="dialogFormEditAbsentVisible"
      top="52px"
      class="custom-dialog absent-end-month"
    >
      <el-form ref="formEditAbsent" :rules="rulesEditAbsent" :model="originFormEditAbsent">
        <!-- ROW 1-->
        <el-row>
          <el-col :span="12">
            <el-form-item label="校舎" prop="snz_businessunit_id">
              <el-input v-model="originFormEditAbsent.snz_businessunit_id" disabled />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item v-role="['system','parttime','employee']" label="作成日">
              <el-date-picker
                v-model="originFormEditAbsent.created_at"
                format="yyyy/MM/dd"
                disabled
                type="date"
                prefix-icon="abc"
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="学年" prop="snz_grade_name">
              <el-input v-model="originFormEditAbsent.snz_grade_name" disabled />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item v-role="['system','parttime','employee']" label="作成者">
              <el-select v-model="originFormEditAbsent.user_id" disabled>
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="生徒コード" prop="snz_studentcode">
              <el-input v-model="originFormEditAbsent.snz_studentcode" disabled />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item v-role="['system','parttime','employee']" class="custom-date" label="修正日">
              <el-date-picker
                v-model="originFormEditAbsent.updated_at"
                disabled
                type="date"
                format="yyyy/MM/dd"
                prefix-icon="abc"
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="生徒氏名" prop="fullname">
              <el-input v-model="originFormEditAbsent.fullname" disabled />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item v-role="['system','parttime','employee']" label="修正者">
              <el-select v-model="originFormEditAbsent.modified_by_user_id" disabled placeholder="">
                <el-option v-for="(item,index) in userCreated" :key="index" :label="item.name" :value="item.id" />
              </el-select>
            </el-form-item>
          </el-col>
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item
              ...
              :rules="[
                {
                  required: Boolean(originFormEditAbsent.status),
                  message: '休塾開始月を選択してください', trigger: 'change'
                },
              ]"
              prop="snz_startmonth_period_id"
            >
              <search-select-box
                v-model="originFormEditAbsent.snz_startmonth_period_id"
                :search-function="searchTargetPeriod"
                :default-data="dataTargetPeriod"
                key-name="snz_name"
                :append-query="{pattern_id: originFormEditAbsent.patternId}"
                placeholder=""
          ...
          <el-col :span="12">
            <el-form-item v-role="['system']" label="ステータス" prop="status">
              <el-select v-model="originFormEditAbsent.status" prop="region" placeholder="">
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item class="custom-date" label="休塾終了月" prop="snz_endmonth_period_id">
              <search-select-box
                v-model="originFormEditAbsent.snz_endmonth_period_id"
                :search-function="searchTargetPeriodEnd"
                :default-data="dataTargetPeriod"
                key-name="snz_name"
                :append-query="{pattern_id: originFormEditAbsent.patternId}"
                placeholder=""
          ...
          <el-col :span="12">
            <el-form-item
              ...
              :rules="[
                {
                  required: Boolean(originFormEditAbsent.status),
                  message: '休塾理由を選択してください', trigger: 'change'
                },
              ]"
              prop="snz_absentreasonoption"
            >
              <el-select v-model="originFormEditAbsent.snz_absentreasonoption" clearable placeholder="">
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="休塾理由その他" prop="snz_absentreasonother">
              <el-input v-model="originFormEditAbsent.snz_absentreasonother" />
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
      <span slot="footer" class="dialog-footer custom-footer__button">
        ...
        <el-button v-if="ableRole(['system'])" class="delete" @click="deleteAbsent">
...
export default {
  ...
  methods: {
    closeModal() {
      this.$emit('closeEditAbsent');
    },
    saveStartAbsentClick() {
      this.$emit('saveEditAbsentClick');
    },
    deleteAbsent() {
      this.$emit('deleteEditAbsent');
    },
    async searchTargetPeriod(query) {
      return await targetPeriodRequest.searchName(query);
    },
    async searchTargetPeriodEnd(query) {
      return await targetPeriodRequest.searchName(query);
    },
    handleChange(prop) {
      this.$refs.formEditAbsent.clearValidate(prop);
    },
  },
};
</script>
