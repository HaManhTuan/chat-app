      ...
      <el-tabs v-model="activeTab" type="border-card" class="tab-custom">
        <el-tab-pane :key="1" label="全般" name="general">
          <el-scrollbar class="scroll-content">
            <general-component v-if="dataLoaded" ref="generalComponent" :target-period="item" />
          </el-scrollbar>
        </el-tab-pane>
        <el-tab-pane :key="2" label="システム" name="system">
          <el-scrollbar class="scroll-content">
            <system-component ref="systemComponent" :target-period="item" />
          </el-scrollbar>
        </el-tab-pane>
      </el-tabs>
...
export default {
      ...
      if (!_.isEmpty(this.dataOriginal) && !_.isEqual(this.dataOriginal, dataAll)) {
        this.$confirm('このページを終了する前に、変更を保存しますか', '保存されていない変更', {
          ...
          if (!this.isError && typeof this.$route.params.id !== 'undefined') {
            targetPeriodRequest.update(this.$route.params.id, dataAll).then(() => {
          ...
          if (!this.isError && typeof this.$route.params.id === 'undefined') {
            targetPeriodRequest.store(dataAll).then(() => {
              this.generalFormError = false;
              Message({
                message: this.CommonTxt.saveSuccess,
                type: 'success',
  ...
  methods: {
    async getData() {
      this.item = await targetPeriodRequest.get(this.$route.params.id);
      this.dataOriginal = JSON.parse(JSON.stringify(this.item));
    },
    async saveData() {
      const dataGen = this.$refs.generalComponent.formData;
    ...
    deleteData(saveDelete = false) {
      this.$confirm(this.CommonTxt.deleteConfirmTxt, '警告', {
        ...
        await targetPeriodRequest.destroy(this.$route.params.id).then(() => {
          Message({
          ...
          this.redirectPage({ 'name': 'TargetPeriod', params: { redirect: true }, query: query });
        }).catch((err) => {
          if (err.response.status === 424) {
            Message({
              message: this.CommonTxt.deleteNotPermission,
    ...
    async update(dataAll){
      await targetPeriodRequest.update(this.$route.params.id, dataAll).then(res => {
        this.generalFormError = false;
        Message({
          message: this.CommonTxt.saveSuccess,
          type: 'success',
        ...
        if (this.$refs.systemComponent.$data.formData.status !== 1) {
          this.dataOriginal = {};
          this.redirectPage({ 'name': 'TargetPeriod', params: { redirect: true }});
        } else {
          this.item = res;
          this.dataOriginal = JSON.parse(JSON.stringify(this.item));
        }
    ...
    async create(dataAll){
      await targetPeriodRequest.store(dataAll).then((res) => {
        this.generalFormError = false;
        this.dataOriginal = {};
        Message({
          message: this.CommonTxt.saveSuccess,
        ...
        this.redirectPage({ name: 'TargetPeriodEdit', params: { id: res.id }});
      }).catch(err => {
        if (err.response.status === 422) {
          this.activeTab = 'general';
          this.generalFormError = true;
